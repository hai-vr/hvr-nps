...
namespace HVR.NPS
{
    public class HVRNPSNPSBoganList
    {
        public object[] items;
        public int size;

        public HVRNPSNPSBoganList(int initialCapacity)
        {
        ...
        public void Add(object item)
        {
            ...
            {
                var newBeacons = new object[items.Length + 10];
                Array.Copy(items, newBeacons, items.Length);
                items = newBeacons;
                Debug.Log($"Capacity changed to {newBeacons.Length}, consider increasing the initial capacity.");
            }
